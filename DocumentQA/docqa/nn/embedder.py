...
class FixedWordEmbedder(WordEmbedder):
    ...
    def init(self,nlp, loader: ResourceLoader, voc: Iterable[str]):
        if self.cpu:
            with tf.device("/cpu:0"):
                self._init(nlp,loader, voc)
        else:
            self._init(nlp,loader, voc)

    def _init(self,nlp, loader: ResourceLoader, voc: Iterable[str]):
        # TODO we should not be building variables here
        if voc is not None:
            word_to_vec = loader.load_word_vec(nlp,self.vec_name, voc)
        else:
