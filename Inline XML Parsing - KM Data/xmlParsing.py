...
for filepath in glob.iglob(r'D:\Inline XML Parsing\XMLs\Dailymed_source\Accepted_Files\Final_11k\*.xml'):
    #print(filepath)
...
# =============================================================================
    ...
    if ((indication.strip()=="" or dosage.strip()=="")):
        print(filepath)
            #log.write(filepath+"\n")
    if re.match(r".*(?:Allergenic extract|allergen).*",indication,re.IGNORECASE | re.DOTALL)==None:
        for ndc in ndcALL:
            query="select APPLICATIONNUMBER,PRODUCTID,product_ndc,ProductCode,[code and ndc],[Product name],[company name], \
                                      ...
                                      LISTING_RECORD_CERTIFIED_THROUGH from temp_24sep_test_new_20nov where product_ndc="+"'"+str(ndc)+"'"
            for row in cursor.execute(query):
                ExList.append([ntpath.basename(filepath),row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],\
                               row[10],row[11],row[12],row[13],row[14],row[15],row[16],row[17],row[18],row[19],indication,dosage])

df = pd.DataFrame(ExList)
df.to_excel('E:/output.xlsx', header=False, index=False)  
